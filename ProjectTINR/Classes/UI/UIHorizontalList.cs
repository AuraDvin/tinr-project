...
public class UIHorizontalList : GameObject, IUiDrawableComponent {
    ...
    public void Initialize(Vector2 position, List<UIButton> buttons) {
        _position = position;
        _buttons = buttons;
        _selectedIndex = 0;
        
        // Load cursor and button background textures
        try {
            _cursorTexture = CreateCursorTexture();
            _buttonBackgroundTexture = CreateButtonBackgroundTexture();
    ...
    private Texture2D CreateCursorTexture() {
        Texture2D texture = new Texture2D(Game.GraphicsDevice, 30, 30);
    ...
    public void Draw(SpriteBatch spriteBatch) {
        ...
        for (int i = 0; i < _buttons.Count; i++) {
            ...
            if (_buttonBackgroundTexture != null) {
                spriteBatch.Draw(
                    ...
                    new Rectangle((int)buttonPos.X, (int)buttonPos.Y, _buttonWidth, _buttonHeight),
                    Color.White);
            }
        ...
        if (_selectedIndex >= 0 && _selectedIndex < _buttons.Count && _cursorTexture != null) {
            Vector2 selectedButtonPos = _buttons[_selectedIndex].TexturePosition;
            Vector2 cursorPos = new Vector2(
                selectedButtonPos.X - 40,
                selectedButtonPos.Y + (_buttonHeight - 30) / 2);
            
            spriteBatch.Draw(
                _cursorTexture,
                new Rectangle((int)cursorPos.X, (int)cursorPos.Y, 30, 30),
                Color.White);
        }
    }
