...
public class RectCollisionShape(bool isStatic) : ICollisionShape, IMoveComponent {
    ...
    public virtual bool OnCollision(ICollisionShape other) {
        ...
        if (other is FloorCollisionShape floor) {
            ...
            float overlapX = Math.Min(floorRect.Right, Rectangle.Right) - Math.Max(floorRect.Left, Position.X);
            float overlapY = Math.Min(floorRect.Bottom, Rectangle.Bottom) - Math.Max(floorRect.Top, Position.Y);

            if (overlapX < overlapY) {
                OnWall = true;
                if (Rectangle.Center.X < floorRect.Center.X) {
                    // Wall is on the right
                    Velocity = new(Math.Min(Velocity.X, 0), Velocity.Y);
                    Owner.Position = new Vector2(floorRect.Left - Rectangle.Width - Offset.X + 1, Owner.Position.Y);
                }
                else {
                    // Wall is on the left
                ...
                if (Rectangle.Center.Y < floorRect.Center.Y) {
